    ...
    public SecurityFilterChain configureAuthorizationFilterChain(HttpSecurity http) throws Exception {
        http.csrf(AbstractHttpConfigurer::disable).cors(cors -> cors.configurationSource(this.corsConfigurationSource))
                .authorizeHttpRequests(auth -> auth
                        ...
                        .requestMatchers("/api/v3/api-docs/**").permitAll() //.hasAuthority(AuthorityLevel.ADMIN)
                        .requestMatchers("/api/swagger-ui/**").permitAll() //.hasAuthority(AuthorityLevel.ADMIN)
                        .requestMatchers("/api/swagger-ui.html").permitAll() //.hasAuthority(AuthorityLevel.ADMIN)
